"""
Module created as part of HW_2_3 for preprocessing Bank Customer Churn Prediction dataset
"""
...
def preprocess_data(
    ...
    scaler_mode: str = "minmax",
    encoder_drop: Optional[str] = "if_binary",
    random_state: int = 42,
) -> Dict[
    str,
    Union[
        ...
        List[str],
        SimpleImputer,
        Union[MinMaxScaler, StandardScaler, None],
        OneHotEncoder,
    ],
]:
    """
...
def preprocess_new_data(
    ...
    scaler: Optional[Union[MinMaxScaler, StandardScaler]],
    encoder: OneHotEncoder,
    drop_cols: Optional[List[str]] = None,
) -> pd.DataFrame:
    """
    Preprocess new/unseen data for prediction using fitted transformers.

    Parameters
    ----------
    df : pd.DataFrame
        New dataset (e.g., test set).
    imputer : SimpleImputer
        Fitted imputer from training.
    scaler : MinMaxScaler or StandardScaler or None
        Fitted scaler from training. If None, scaling is skipped.
    encoder : OneHotEncoder
