...
# todo - remove and use above create_binary_flag instead
def transformBalance(
    train_inputs, val_inputs, numeric_cols, drop=True, test_inputs=None
):
    ...
    train_inputs[zero_balance_col] = (
        train_inputs["Balance"] == train_inputs["Balance"].min()
    ).astype(int)
    val_inputs[zero_balance_col] = (
        val_inputs["Balance"] == val_inputs["Balance"].min()
    ).astype(int)
    if test_inputs:
        test_inputs[zero_balance_col] = (
            test_inputs["Balance"] == test_inputs["Balance"].min()
        ).astype(int)
    ...
    if drop:
        ...
        return (
            train_inputs.drop("Balance", axis=1),
            val_inputs.drop("Balance", axis=1),
            test_inputs,
            numeric_cols,
        )
